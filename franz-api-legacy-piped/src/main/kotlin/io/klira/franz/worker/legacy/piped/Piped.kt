package io.klira.franz.worker.legacy.piped

import io.klira.franz.*
import mu.KotlinLogging

private typealias Predicate<U> = suspend (U) -> Boolean
private typealias WorkerFunction<U> = suspend (JobState<U>) -> JobStatus
private typealias SideEffect<U> = suspend (U) -> Unit
private typealias Transform<U, R> = suspend (U) -> R

enum class JobStatus : JobUpdate {
    Success,
    TransientFailure,
    PermanentFailure,
    Incomplete,
    ...
    override fun mayAdvanceOffset(): Boolean = isDone()
}
...
abstract class LegacyPipedWorker(private val interceptors: List<WorkerInterceptor>) : Worker {
    init {
        setupInterceptors()
    }
    ...
    override fun batchType(): JobBatchType = JobBatchType.LINEAR
    override suspend fun processBatch(jobBatch: JobBatch): List<Pair<Job, JobUpdate>> =
        jobBatch.asLinear().map {
            it to JobState(it.message(), interceptors)
        }.map { (job, js) ->
            job to performWorked(js)
        }

    abstract fun performWorked(js: JobState<Message>) : JobStatus
